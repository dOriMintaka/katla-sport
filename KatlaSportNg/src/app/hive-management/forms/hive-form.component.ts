...
export class HiveFormComponent implements OnInit {
  ...
  onDelete() {
    this.hiveService.setHiveStatus(this.hive.id, true).subscribe(i => this.hive.isDeleted = true);
  }

  onUndelete() {
    this.hiveService.setHiveStatus(this.hive.id, false).subscribe(i => this.hive.isDeleted = false);
  }

  onPurge() {
    this.hiveService.deleteHive(this.hive.id).subscribe(i => this.navigateToHives());
  }
}
